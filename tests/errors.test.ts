...
// `toError` is internal (not re-exported) but exercised here for its branches.
...
describe("toError (internal)", () => {
    it("passes through an existing Error (same reference)", () => {
        const err = new Error("boom");
        expect(toError(err)).toBe(err);
    });

    it("wraps a string as an Error preserving the message", () => {
        const err = toError("boom");
        expect(err).toBeInstanceOf(Error);
        expect(err.message).toBe("boom");
    });

    it("formats a number as 'unknown error'", () => {
    ...
    it("formats an object as 'unknown error'", () => {
        expect(toError({ weird: true }).message).toBe("unknown error");
    });
});
